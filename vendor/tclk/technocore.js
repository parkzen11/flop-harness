...
function requireContractId(contract) {
 ...
 */
export function capabilityToken(rails) {
    if (rails.length === 0)
        throw new Error("tclk: capability token needs at least one rail");
    return `tclk1:${normalizeRailIds(rails).join(",")}`;
}
/** Parse the capability token from a DID-note value. Null when absent or malformed. */
export function parseCapabilityToken(note) {
 ...
 */
export function stateNote(contract) {
...
export function parseStateNoteValue(value) {
    ...
    if (rest.length > 0 || !STATUSES.has(status))
        return null;
    if (railRef === undefined)
        return { status: status };
    return RAIL_REF.test(railRef) ? { status: status, railRef } : null;
}
