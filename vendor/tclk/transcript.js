...
import { applyFrame, openContract } from "./machine.js";
...
export function verifyTranscriptRecord(record) {
    ...
    if (typeof record.line !== "string")
        return invalid("record line must be a string");
    if (typeof record.sender !== "string")
        return invalid("record sender must be a string");
    if (record.nonce === null || record.signature === null) {
        return invalid("record is unsigned");
    }
    if (!NONCE.test(record.nonce))
        return invalid("record nonce is not canonical decimal");
    ...
    if (publicKey === null)
        return invalid("record sender is not an Ed25519 did:key");
    try {
...
export function transcriptRecord(room, value) {
    if (!ROOM_NAME.test(room))
        throw new Error(`tclk: invalid transcript room ${JSON.stringify(room)}`);
    ...
    if (typeof message.ts !== "string")
        throw new Error("tclk: transcript message has no timestamp");
    ...
    if (typeof message.from !== "string")
        throw new Error("tclk: transcript message has no sender");
    if (typeof message.text !== "string")
        throw new Error("tclk: transcript message has no text");
    let nonce = null;
...
export function parseTranscriptExport(room, jsonl) {
    const records = [];
    jsonl.split("\n").forEach((line, index) => {
        if (line.trim() === "")
            return;
...
function authenticatedFrame(record) {
    if (record.room !== OFFER_ROOM || !verifyTranscriptRecord(record).ok)
        return null;
 ...
 */
export function findContractHandshake(records, contract) {
    ...
    for (const record of records) {
        ...
        if (frame?.type === "offer") {
            if (!offers.has(frame.id))
                offers.set(frame.id, record);
            continue;
        }
        if (frame?.type !== "accept" || frame.contract !== contract)
            continue;
        const offer = offers.get(frame.ref);
        if (offer !== undefined)
            return { offer, accept: record };
